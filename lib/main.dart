  ...
  Widget _buildKeypad() {
    return Column(
      children: [
        ...
        // Optional memory buttons row
        Expanded(
          child: Row(
            children: [
              _buildButton("MC", action: () => _memory = 0),
              _buildButton("MR", action: () => _appendExpression(_memory.toString())),
              _buildButton("M+", action: () => _memory += double.tryParse(_result) ?? 0),
              _buildButton("M-", action: () => _memory -= double.tryParse(_result) ?? 0),
            ],
          ),
        ),
            ...
            children: [
              ...
              if (!_isPremium)
                ListTile(
                  ...
                  subtitle: Text(
                      _isRewardedReady ? "Watch ad to unlock" : "Ad loading..."),
